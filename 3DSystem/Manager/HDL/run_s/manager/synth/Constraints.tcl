...
#---------------------------------------------------------
 ...
 if {($modname == "dfi") || ($modname == "manager")} {
   set_clock_uncertainty $CLK_SKEW [format "%s%s" clk_diram" "2x" ]
   set_clock_uncertainty $CLK_SKEW [format "%s%s" clk_diram" ""   ]
   set_clock_uncertainty $CLK_SKEW [format "%s%s" clk_diram" "_cq"]
 }
...
if {$tech == "65nm"} {
 ...
 if {($modname == "dfi") || ($modname == "manager")} {
   set_input_delay $IP_DELAY -clock [format "%s%s" clk_diram" "2x" ] [remove_from_collection [all_inputs] [format "%s%s" clk_diram" "2x" ]]
   set_input_delay $IP_DELAY -clock [format "%s%s" clk_diram" ""   ] [remove_from_collection [all_inputs] [format "%s%s" clk_diram" ""   ]]
   set_input_delay $IP_DELAY -clock [format "%s%s" clk_diram" "_cq"] [remove_from_collection [all_inputs] [format "%s%s" clk_diram" "_cq"]]
 }
 ...
 if {($modname == "dfi") || ($modname == "manager")} {
   set_input_delay 0.0060452 -clock [format "%s%s" clk_diram" "2x" ] [remove_from_collection [all_inputs] [format "%s%s" clk_diram" "2x" ]]
   set_input_delay 0.0060452 -clock [format "%s%s" clk_diram" ""   ] [remove_from_collection [all_inputs] [format "%s%s" clk_diram" ""   ]]
   set_input_delay 0.0060452 -clock [format "%s%s" clk_diram" "_cq"] [remove_from_collection [all_inputs] [format "%s%s" clk_diram" "_cq"]]
 }
...
if {$tech == "65nm"} {
 ...
 if {($modname == "dfi") || ($modname == "manager")} {
    set_driving_cell -lib_cell "$DR_CELL_NAME" -pin "$DR_CELL_PIN" [remove_from_collection [all_inputs] [format "%s%s" clk_diram" "2x" ]]
    set_driving_cell -lib_cell "$DR_CELL_NAME" -pin "$DR_CELL_PIN" [remove_from_collection [all_inputs] [format "%s%s" clk_diram" ""   ]]
    set_driving_cell -lib_cell "$DR_CELL_NAME" -pin "$DR_CELL_PIN" [remove_from_collection [all_inputs] [format "%s%s" clk_diram" "_cq"]]
 }
 ...
 if {($modname == "dfi") || ($modname == "manager")} {
    set_driving_cell -lib_cell $DFF_CELL_NAME -pin $DFF_CELL_PIN [remove_from_collection [all_inputs] [format "%s%s" clk_diram" "2x" ]]
    set_driving_cell -lib_cell $DFF_CELL_NAME -pin $DFF_CELL_PIN [remove_from_collection [all_inputs] [format "%s%s" clk_diram" ""   ]]
    set_driving_cell -lib_cell $DFF_CELL_NAME -pin $DFF_CELL_PIN [remove_from_collection [all_inputs] [format "%s%s" clk_diram" "_cq"]]
 }
