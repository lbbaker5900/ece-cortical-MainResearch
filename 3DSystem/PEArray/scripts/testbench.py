...
if __name__ == "__main__":
  ...
  for lane in range (0, numOfExecLanes):
    ...
    #pLine = pLine + '\n                                                                                                                                                  '.format(lane) 
    #pLine = pLine + '\n                  sys_operation_lane_gen[{0}]             =  new sys_operation_lane[{0}]  ;  // seed object. Dont use directly as all lanes will use the same operation'.format(lane)
    #pLine = pLine + '\n                  //sys_operation_lane_gen.setPriorOperations(priorOperations)   ;  // object may need to know what went before'.format(lane)
    #pLine = pLine + '\n '
    #pLine = pLine + '\n                  sys_operation_lane_gen[{0}].c_operationType_definedOrder .constraint_mode(0) ;'.format(lane)
    #pLine = pLine + '\n                  sys_operation_lane_gen[{0}].c_operationType_all          .constraint_mode(0) ;'.format(lane)
    #pLine = pLine + '\n                  sys_operation_lane_gen[{0}].c_operationType_fpMac        .constraint_mode(1) ;'.format(lane)
    #pLine = pLine + '\n                  sys_operation_lane_gen[{0}].c_operationType_copyStdToMem .constraint_mode(0) ;'.format(lane)
    #pLine = pLine + '\n                  //'.format(lane)
    #pLine = pLine + '\n                  sys_operation_lane_gen[{0}].c_streamSize.constraint_mode(1)                 ;'.format(lane)
    #pLine = pLine + '\n                  sys_operation_lane_gen[{0}].c_operandValues.constraint_mode(1)              ;'.format(lane)
