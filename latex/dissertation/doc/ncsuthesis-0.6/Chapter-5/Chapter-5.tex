...
Therefore, the system must decode instructions, configure the various functions, pre-fetch and pipeline \ac{dram} data and perform the actual activation calculation. To achieve high average \ac{dram} bandwidth usage, these functions are all performed in parallel and this work demonstrates a average system bandwidth able to meet the ten's of \SI[per-mode=symbol]{16}{\tera \bit \per \second} of bandwidth requirement outlined in equation \eqref{eq:averageBandwidth}.
