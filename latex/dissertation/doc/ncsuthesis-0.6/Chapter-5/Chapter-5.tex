...
When performing inference in \ac{ann}, the computational hotspot is the \ac{an} activation calculation shown in figure \ref{fig:Rate Based Model} and equation \eqref{eq:activation function}.
This \ac{an} activation involves hundreds or thousands of multiply-accumulates of \ac{an} activations and corresponding connection weights. 
In this work, the \ac{an} activations and weights are stored in \ac{dram}. 
Therefore, because of the complex access protocol associated with \ac{dram}, one of the main objectives is to demonstrate the 3DDRAM can be accessed with relatively high levels of bus efficiency.

The system has to process thousands of \acp{an} concurrently and do this with minimal unused bus cycles.
Therefore, the system must decode instructions, configure the various functions, pre-fetch and pipeline \ac{dram} data and perform the actual activation calculation. To achieve high average \ac{dram} bandwidth usage, these functions are all performed in parallel and this work demonstrates a average system bandwidth able to meet the tens of \SI[per-mode=symbol]{16}{\tera \bit \per \second} of bandwidth requirement outlined in equation \eqref{eq:averageBandwidth}.
...
This work employs \ac{3dic} technology along with a customized 3D-DRAM. The objective was to demonstrate that a pure \ac{3dic} system can implement multiple disparate \acp{ann}. By staying within the \ac{3dic} footprint and taking advantage of high
...
\begin{itemize}
  \item Computations requiring single precision floating point precision
  \item Tezzaron \acf{diram4} \cite{tezzaron:diram4} for main memory
\end{itemize}
