...
The operation tuple is decoded and a \ac{stop} pointer and a \ac{simd} \ac{pc} pointer are sent to the \ac{pe} inside an \ac{oob} control packet.
The \ac{stop} pointer specifies what streaming operation is to take place on the data directly streamed to the \ac{pe}. 
In the baseline system, typically this would be a \ac{fp} \ac{mac} on the pre-synaptic \ac{an} states and connection weights provided on the two streams in each execution lane.
