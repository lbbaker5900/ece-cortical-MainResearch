...
The \ac{stop} pointer is used to index into a small \ac{sram} that contains the long instruction word used to select the \ac{stop} function and control the source and destination of the data.
The contents of the memory is shown in Figure \ref{fig:stOp pointer memory} and Table \ref{tab:stOp pointer memory}.
...
\captionsetup{width=0.9\textwidth}
  ...
  \begin{minipage}{1\textwidth}
    ...
    \vspace{-1.0cm}
    \captionsetup{justification=centering}%, skip=10pt}
    \captionof{figure}{stOp pointer memory contents}
    \label{fig:stOp pointer memory}
    \vspace{2mm}
  ...
  \begin{minipage}{0.80\textwidth}
    ...
    \begin{adjustbox}{width=1\textwidth}
        ...
        \begin{tabular}{ |c|c|c|  }
          ...
          9     & Send null                      & Send a null packet upstream                         \\\cline{1-3}
         10     & Clear local register           &  \multirow{4}{*}{no data operations}                \\\cline{1-2}
         11     & Clear common register          &                                                     \\\cline{1-2}
         12     & Clear index register           &                                                     \\\cline{1-2}
         13     & Increment index register       &                                                     \\\cline{1-3}
        14-15   & \multicolumn{2}{c|}{NA}                                                              \\\cline{1-3}
          \hline
        \end{tabular}
    \end{adjustbox}
...
In the case of the \ac{stop}, one of the \ac{stop} inputs can be sourced from the local memory using a \ac{dma} controller.
...
The upstream packet format accommodates both data and tag to be transmitted or just the tag.
The reason for allowing tag-only operation is to accommodate a sync flush operation without data being returned to the manager.
...
\begin{landscape}
  ...
  \begin{figure}[!htbp]
    ...
    \captionsetup{width=0.9\textwidth}
    \centerline{
    \mbox{\includegraphics[angle=0, width=1.0\linewidth]{DetailedBlockDiagram}}
    }
    \caption{Sub-System Column (SSC) Block Diagram}
