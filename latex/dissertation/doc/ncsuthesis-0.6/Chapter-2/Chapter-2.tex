...
\captionsetup{justification=centering, skip=10pt}
\caption{Layer to Layer Connection Types}
\label{fig:Layer to Layer Connection Types}
\end{figure}
...
The feature kernels employed in the locally-connected layers can be unique to the regions of the previous layer or the same filter kernels can be employed across the entire previous layer.
...
An example of a \ac{cnn} can be seen in figure \ref{fig:DNN showing layer order} with the layer configurations shown in table \ref{tab:Layer Configuration}. A \ac{cnn} similar to this has demonstrated high levels of efficacy in image recognition applications.
