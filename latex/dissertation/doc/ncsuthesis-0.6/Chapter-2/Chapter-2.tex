...
Assuming these complex functions will require \ac{ann}s similar in size to figure \ref{fig:Typical DNN} and \cite{krizhevsky2012imagenet}, these implementations will be processing multiple large \ac{ann}s at or near real-time.
...
As a metric, this work assumes that any useful \ac{ann} will be similar to \ref{fig:Typical DNN} which utilizes $>\num{900e3}$\acp{an} and $\approx\num{200e6}$ parameters. 
...
\begin{table}[h*]
  ...
  \centering
    ...
    \vspace{3pt}
\end{table}
Given the bandidth and storage requirements, the problem becomes \hyphenquote{american}{\textbf{\textcolor{black}{to provide deterministic at or near real-time performance within tolerable power and space constraints for edge systems employing inference on multiple disparate useful-sized neural networks.}}}
%\hlc[gray]{hello}
