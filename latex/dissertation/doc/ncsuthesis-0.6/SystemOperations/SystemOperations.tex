...
There are tasks in the system that must be performed by chaining instructions together.
For example, to load the \ac{simd} instruction memory requires the procedure described in algorithm \ref{alg:Load SIMD Instruction memory}.
...
\\
    \State{// Host starts simd instruction download to \ac{ssc} memory}
    \State{// Next \ac{ssc} instruction prepares wr_cntl for data from Host}
