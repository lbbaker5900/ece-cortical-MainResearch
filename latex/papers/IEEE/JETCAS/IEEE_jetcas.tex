...
\IEEEPARstart{U}{seful} \acp{dnn}s often require hundreds of thousands of \acp{an}.
...
So in these \ac{ann}s, the memory requirements are significant. The storage is required for the input, the \ac{an} state and most significantly the pre-synapic \ac{an} connection weights for each of the \acp{an}. This storage requirement often results in gigabytes of memory.
...
In most cases, Graphics processing Units (GPU) are used to implement large \ac{ann}s. In many \ac{ann} architectures, such as Convolutional \ac{ann}s (\ac{cnn}), they are quite effective. 
...
Implementations such as Neurostream \cite{azarkhish2017neurostream}, NeuroCube\cite{kim2016neurocube} and Tetris \cite{gao2017tetris} employ \ac{hmc} \ac{3ddram} in a \ac{3dic} system, and all have demonstrated improvements over non-\ac{3dic} systems and \acp{gpu}.
...
In additon, as \acp{ann} fulfill their potential and users start to employ them in various applications, there is no reason to believe these applications will exhibit the same stationary characteristcs that make \acp{cnn} effective in image processing.
...
As a metric, this work assumes that any useful \acp{dnn} will employ 100's of thousands of \acp{an} \cite{krizhevsky2012imagenet}. 
...
Assuming that any useful \ac{ann} will be of a similar size to \cite{krizhevsky2012imagenet}, this work employs a baseline \ac{ann} similar to that shown in Table \cite{tab:Baseline Layer Configuration},
...
A system employing 10 \ac{ann}s for various disparate functions and an average processing time of \SI{16}{\milli\second} will require an average or ``goldilocks'' bandwidth of \SI[per-mode=symbol]{26}{\tera \bit \per \second} \eqref{eq:maximumBandwidth} and a capacity of \SI[per-mode=symbol]{8.0}{\giga\byte} \eqref{eq:memoryRequired}.
\begin{figure*}[!htbp]
...
By employing a \ac{3dic} system along with customizations to a standard \ac{3ddram}, this work demonstrates a system that is able implement multiple useful sized \acp{dnn}s whilst maintaining the required average (``goldilocks'') memory bandwidth of \SI[per-mode=symbol]{26}{\tera \bit \per \second}.
...
Although not optimized for specific \ac{ann}s, such as \acp{cnn}, this work demonstrates the potential for real-time performance at the embedded when implementing fully connected \acp{dnn}s or other similar \ac{ann}s such as \ac{lstm}.
...
\iffalse
Even though the baseline system focuses on the \ac{an} multiply-accumulate followed by a ReLu activation function, the system has built in flexibility into the stOp function to allow other functions to be added
\fi

In most cases, the stOp module will operate on the \ac{an} state and weights provided by the manager and provide the result to the SIMD.
% ----------------------------------------------------------------------------------------------------
\subsubsection{SIMD}
\label{ssec:SIMD}
...
\iffalse
When processing an \ac{ann}, to compute the activation of an individual \ac{an} involves reading the pre-synaptic \ac{an} activation's and the weights of the connections between the pre-synaptic \acp{an} and the \ac{an} being processed.
\fi
For every group of \acp{an} processed, the state of the group of \acp{an} is written back to memory.

\iffalse
Because of the high average pre-synaptic fanin, the ratio of reading \ac{an} states and connection weights to writing back the \ac{an} states is high, 100's or 1000's to one. 
