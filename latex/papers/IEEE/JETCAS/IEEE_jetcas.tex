...
CNNs assume a significant amount of reuse of the weights connecting ANs and thus they can take advantage of local memory (SRAM).
...
The work in this paper considers "edge" applications which perform multiple complex functions, such as navigation, engine monitoring etc. for systems employed "in the field".
The target applcation assumes that a) there is no access to cloud servers, b) the system processes a disparate set of useful sized ANNs and c) there are not opportuities to store and reuse portions of the ANN in SRAM. 
Therefore this work makes no assumptions regarding opportuities to store and reuse portions of the ANN in local SRAM, such as with CNNs or batch processing.
These requirements dictate the need for DRAM for ANN storage and a sustained average bandwidth of 10's of \SI[detect-weight,per-mode=symbol]{}{\tera \bit \per \second} when accessing that DRAM.

This work employs 3D integrated circuit technology and a proposed custom 3D-DRAM. By employing 3DIC technology, this work takes advantage of the reduced energy and area and increased connectivity and bandwidth to allow the DRAM to be employed efficiently.
This work demonstrates the required average bandwidth exceeding  10's of \SI[detect-weight,per-mode=symbol]{}{\tera \bit \per \second} when run against various fully and locally connected ANN layers providing a potentially 10-50X performance improvement over existing ASIC/ASIPs or GPUs.

\iffalse
 where there may not be opportuities to store and reuse portions of the ANN in SRAM. A further assumption is that the target edge devices do not include opportunities to perform batch processing.

This work also does not assume the ANN is specifically a CNN but a DNN where there may not be opportuities to store and reuse portions of the ANN in SRAM. A further assumption is that the target edge devices do not include opportunities to perform batch processing.
...
This work uses the DRAM as the primary processing storage 
\fi

\iffalse
a collection of testcases which  which is requred to support multiple disparate ANNs for at or near real-time performance.
The bus efficiency is the main metric, and demonstrating sustained average bandwidth over a collection of fanins was the goal.
...
\iffalse
The SSC includes the DRAM port, the manager and the PE. 
\fi
...
\iffalse
The manager will store these back to DRAM most likely in the array format as described earlier.
...
\iffalse
When the system writes the result of an operation back to memory, it is typically writing a small portion of a DRAM page and the nature of the DRAM protocol means this is a very inefficient use of DRAM bandwidth. 
So although the amount of data written is small the performance impact cannot be ignored.
\fi
...
\iffalse
When processing an ANN, to compute the activation of an individual AN involves reading the pre-synaptic AN activation's and the weights of the connections between the pre-synaptic ANs and the AN being processed.
\fi
  ...
  \begin{subtable}{0.5\textwidth}
    ...
    \begin{adjustbox}{width=0.85\textwidth}
      \begin{tabular}{cccc}
        ...
        \hline  % instead of \midrule %midrule doesnt overlap with column lines
                   28nm  & \SI{500}{\mega\hertz} &   48W    &  CONV-294\iffalse \SI[per-mode=symbol]{\sim 70}{\percent} \fi \\ %\cline{2-2}
                   28nm  & \SI{700}{\mega\hertz} &   65W    &  CONV-294\iffalse \SI[per-mode=symbol]{\sim 70}{\percent} \fi \\ %\cline{2-2}
        \bottomrule
      \end{tabular}
    \end{adjustbox}
    \vspace{3pt}
  ...
  \centering
    \begin{adjustbox}{width=0.37\textwidth}
      \begin{tabular}{ccc}
        \toprule
                            % \multicolumn{4}{c}{3D-DRAM Simulation-based estimates}   \\
                                                       & \iffalse Downstream                              &\fi                                        \multicolumn{2}{c}{Average Bandwidth}                      \\  %\cline{1-1}
                                                       & \iffalse Stack                                   &\fi                                        \multicolumn{2}{c}{At Frequency}                                      \\  %\cline{1-1}
                   Test                                & \iffalse Bus Efficiency                          &\fi       \SI{500}{\mega\hertz}                            & \SI{700}{\mega\hertz}                               \\  %\cline{1-1}
        \hline  % instead of \midrule %midrule doesnt overlap with column lines                                       
                   CONV2 \cite{krizhevsky2012imagenet} &\iffalse \SI[per-mode=symbol]{\sim 65}{\percent}  &\fi \SI[per-mode=symbol]{\sim 43}{\tera\bit\per\second}    & \SI[per-mode=symbol]{\sim 60}{\tera\bit\per\second} \\ %\cline{2-2}
                   CONV-294                            &\iffalse \SI[per-mode=symbol]{\sim 67}{\percent}  &\fi \SI[per-mode=symbol]{\sim 44}{\tera\bit\per\second}    & \SI[per-mode=symbol]{\sim 61}{\tera\bit\per\second} \\ %\cline{2-2}
                   CONV-300                            &\iffalse \SI[per-mode=symbol]{\sim 73}{\percent}  &\fi \SI[per-mode=symbol]{\sim 48}{\tera\bit\per\second}    & \SI[per-mode=symbol]{\sim 67}{\tera\bit\per\second} \\ %\cline{2-2}
                   CONV-500                            &\iffalse \SI[per-mode=symbol]{\sim 82}{\percent}  &\fi \SI[per-mode=symbol]{\sim 54}{\tera\bit\per\second}    & \SI[per-mode=symbol]{\sim 75}{\tera\bit\per\second} \\ %\cline{2-2}
                   CONV-1000                           &\iffalse \SI[per-mode=symbol]{\sim 89}{\percent}  &\fi \SI[per-mode=symbol]{\sim 58}{\tera\bit\per\second}    & \SI[per-mode=symbol]{\sim 82}{\tera\bit\per\second} \\ %\cline{2-2}
                   FC-350                              &\iffalse \SI[per-mode=symbol]{\sim 78}{\percent}  &\fi \SI[per-mode=symbol]{\sim 51}{\tera\bit\per\second}    & \SI[per-mode=symbol]{\sim 72}{\tera\bit\per\second} \\ %\cline{2-2}
                   FC-500                              &\iffalse \SI[per-mode=symbol]{\sim 84}{\percent}  &\fi \SI[per-mode=symbol]{\sim 55}{\tera\bit\per\second}    & \SI[per-mode=symbol]{\sim 77}{\tera\bit\per\second} \\ %\cline{2-2}
                   FC-1000                             &\iffalse \SI[per-mode=symbol]{\sim 91}{\percent}  &\fi \SI[per-mode=symbol]{\sim 60}{\tera\bit\per\second}    & \SI[per-mode=symbol]{\sim 83}{\tera\bit\per\second} \\ %\cline{2-2}
                   FC-7 \cite{krizhevsky2012imagenet}  &\iffalse \SI[per-mode=symbol]{\sim 94}{\percent}  &\fi \SI[per-mode=symbol]{\sim 62}{\tera\bit\per\second}    & \SI[per-mode=symbol]{\sim 86}{\tera\bit\per\second} \\ %\cline{2-2}
        \bottomrule
      \end{tabular}
    \end{adjustbox}
    \vspace{3pt}
